...
`define CORDIC_1 33'd2593080208
//               33'd259308020802914        // CORDIC inverse
...
//  input wire [`ITERATION_BITS:0] i,
//  input wire signed [`XY_BITS:0] D,
//  output reg signed [`XY_BITS:0] Q );
//  integer j;
//  always @ * begin
//    Q = D;
//    for(j=0;j<i+1;j=j+1) Q = (Q >> 1) | (D[`XY_BITS] << `XY_BITS);
//  end
//endmodule
/*  Rotator
...
//  reg repeat_4;
//  reg repeat_13;
//  reg [`ITERATION_BITS:0] iteration_sh;
  reg repeat_3;
  reg repeat_12;
...
//    repeat_13 <= 0;
//    end
//    else if(repeat_4 == 0 && iteration == 4)
//    begin
//    repeat_4  <= 1;
//    iteration <= 4;
//    end
//    else if(repeat_13 == 0 && iteration == 13)
//    begin
//    repeat_13 <=1;
//    iteration <=13;
//    end
...
//    end
    ...
    else if(repeat_3 == 0 && iteration == 3)
    begin
    repeat_3  <= 1;
    iteration <= 3;
    end
    else if(repeat_12 == 0 && iteration == 12)
    begin
    repeat_12 <=1;
    iteration <=12;
    end
...
//    if (init) done <= 0;
/*    else*/ if (rst) done <= 0;
    else if (iteration == (`ITERATIONS-1)) done <= 1;
    else done<=0;
  rotator_16 U (clk,rst,init,iteration,tanangle(iteration),x,y,z,x_o,y_o,theta_o);
